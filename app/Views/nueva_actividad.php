...
<div class="row justify-content-between">
        <div class="col-2">
          <br>
          <a href="<?=base_url("Home/index");?>" type="button" class="btn btn-danger">Cancelar</a>
        </div>
      </div>
...
<div class="row">
  <div class="col-md-7">
    <div class="mb-3">
      ...
      <input type="text" class="form-control" id="actividad" name="actividad" value="<?=$actividad['actividad'];?>">
      <?php if($validation->getError('actividad')) {?>
        <div class="alert alert-danger mt-2">
        <?= $validation->getError('actividad'); ?>
        </div>
      <?php }?>  
    </div>
    <div class="mb-3">
        ...
        <select class="form-select" id="cliente" name="cliente">
          ...
          foreach($clientes as $cliente){ ?>
              <option value="<?=$cliente->cliente;?>" <?php echo ($actividad['cliente'] == $cliente->cliente) ? 'selected' : '' ?>><?=$cliente->cliente;?></option>
          <?php } ?>
        </select>
        <?php if($validation->getError('cliente')) {?>
        <div class="alert alert-danger mt-2">
        <?= $validation->getError('cliente'); ?>
        </div>
      <?php }?>  
    </div>
    <div class="mb-3">
        ...
        <select class="form-select" id="proyecto" name="proyecto">
          ...
          foreach($proyectos as $proyecto){ ?>
              <option value="<?=$proyecto->proyecto;?>" <?php echo ($actividad['proyecto'] == $proyecto->proyecto) ? 'selected' : '' ?>><?=$proyecto->proyecto;?></option>
          <?php } ?>
        </select>
    ...
    <div class="mb-3">
      <label for="tipo" class="form-label">Tipo <span id="count"></span></label>
      <textarea class="form-control" id="tipo" name="tipo" rows="3"><?=$actividad['tipo'];?></textarea>
    ...
    <div class="mb-3">
      <label for="unidad" class="form-label">Unidad de tiempo <span id="count"></span></label>
      <textarea class="form-control" id="unidad" name="unidad" rows="3"><?=$actividad['unidad'];?></textarea>
      <?php if($validation->getError('unidad')) {?>
        <div class="alert alert-danger mt-2">
        <?= $validation->getError('unidad'); ?>
        </div>
      <?php }?> 
    </div>   
